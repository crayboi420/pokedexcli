...
func commandCatch(args []string, cfg *config) error {
	if len(args) != 1 {
		return errors.New("-catch requires one argument")
	}

	url := "https://pokeapi.co/api/v2/pokemon/" + args[0]

	catchobj, err := ObjReader(cfg, url, CatchData{})

	if catchobj.Name == "" {
		return errors.New("no pokemon like that")
	}
...
func CatchTry(catchobj *CatchData) bool {
	baseExp := catchobj.BaseExperience
	chance := 30.0 / float64(baseExp)
